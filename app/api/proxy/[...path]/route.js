  ...
  try {
    ...
    if (contentType && contentType.includes("text/html")) {
      ...
      return Response.json(
        {
          error:
            "Backend server returned HTML instead of JSON. Check if the API URL is correct.",
          detail:
            "The server at " +
            url +
            " returned an HTML page (likely an error page).",
    ...
    return Response.json(
      {
        ...
        url: url, // Include URL in error for debugging
      },
      { status: 500 }
    );
  }
